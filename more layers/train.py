...
class TrainPipeline():
    ...
    def policy_update(self):
        ...
        explained_var_new = (1 -
                             ...
                             np.var(np.array(winner_batch)))
        print(("lr_multiplier:{:.3f},"
               "loss:{},"
               "entropy:{},"
               "explained_var_old:{:.3f},"
               "explained_var_new:{:.3f}"
               ).format(self.lr_multiplier,
                        loss,
                        entropy,
                        explained_var_old,
                        explained_var_new))
    ...
    def run(self):
        ...
        loss_list = []
        entropy_list = []
        f1=open("loss_list.txt","a")
        f2=open("entropy_list.txt","a")
        f1.write("start!!\n")
        f2.write("start!!\n")
        try:
            for i in range(self.game_batch_num):
                ...
                if len(self.data_buffer) > self.batch_size:
                    ...
                    f1.write(str(loss)+'\n')
                    f2.write(str(entropy)+'\n')
                # check the performance of the current model,
                # and save the model params
                if (i+1) % self.save_freq == 0:
                    path = './model/game_'+str(i)+'.pth'
            ...
            plt.xlabel('games')
            plt.ylabel('entropy')
            plt.plot(range(1, len(entropy_list)+1), entropy_list)
        f1.close()
        f2.close()
        plt.figure(1, figsize=(8, 8))
        plt.xlabel('games')
